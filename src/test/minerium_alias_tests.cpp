// Copyright (c) 2016-2018 The Minerium Core developers
...
BOOST_FIXTURE_TEST_SUITE (minerium_alias_tests, BasicMineriumTestingSetup)
...
BOOST_AUTO_TEST_CASE (generate_big_aliasdata)
{
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "mineriumtxfund " + varray[0].get_str()));
	varray = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "signrawtransaction " + varray[0].get_str()));
	BOOST_CHECK_NO_THROW(CallRPC("node1", "mineriumsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
	GenerateBlocks(5);
	// activation should fail
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "aliasnew jag2 " + baddata + " 3 0 " + newaddress + " '' '' ''"));
	varray = r.get_array();
	BOOST_CHECK_THROW(r = CallRPC("node1", "mineriumtxfund " + varray[0].get_str()), runtime_error);
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "mineriumtxfund " + varray1[0].get_str()));
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "mineriumtxfund " + varray3[0].get_str()));
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "mineriumtxfund " + varray5[0].get_str()));
...
BOOST_AUTO_TEST_CASE (generate_aliaswitness)
{
	...
	BOOST_CHECK_NO_THROW(CallRPC("node2", "mineriumsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
...
BOOST_AUTO_TEST_CASE (generate_aliaspay)
{
	...
	for (unsigned int i = 0; i < MAX_ALIAS_UPDATES_PER_BLOCK; i++)
	{
		...
		BOOST_CHECK_NO_THROW(CallRPC("node3", "mineriumsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
	}
...
BOOST_AUTO_TEST_CASE (generate_aliasprunewithoffer)
{
	...
	string offerguid = OfferNew("node1", "aliasprunewithoffer", "category", "title", "1", "0.05", "description", "MINR");
...
BOOST_AUTO_TEST_CASE (generate_aliasprunewithcertoffer)
{
	...
	string certofferguid = OfferNew("node1", "aliasprunewithcertoffer", "certificates", "title", "1", "0.05", "description", "MINR", certguid);	
	string offerguid = OfferNew("node1", "aliasprunewithcertoffer", "category", "title", "1", "0.05", "description", "MINR");
...
BOOST_AUTO_TEST_CASE (generate_aliasexpired)
{
	...
	CMineriumAddress aliasAddress(pubKey.GetID());
	...
	BOOST_CHECK_THROW(CallRPC("node2", "offernew aliasexpirednode2 category title 1 0.05 description USD '' MINR false 1 BUYNOW 0 0 false 0 ''"), runtime_error);
	// should fail: new escrow with expired arbiter alias

	BOOST_CHECK_THROW(CallRPC("node2", "escrownew false aliasexpire2node2 aliasexpirednode2 " + offerguid + " 1 true 0 0 25 0.005 0 '' MINR 0 0 ''"), runtime_error);
	// should fail: new escrow with expired alias
	BOOST_CHECK_THROW(CallRPC("node2", "escrownew false aliasexpirednode2 aliasexpire " + offerguid + " 1 true 0 0 25 0.005 0 '' MINR 0 0 ''"), runtime_error);
	...
	BOOST_CHECK_NO_THROW(CallRPC("node1", "offerupdate aliasexpire0 " + offerguid + " category title 100 0.05 description USD false '' 0 MINR BUYNOW 0 0 false 0 ''"));
	GenerateBlocks(5, "node1");
