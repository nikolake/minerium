...
const char * const MINERIUM_CONF_FILENAME = "minerium.conf";
const char * const MINERIUM_PID_FILENAME = "mineriumd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "minerium" is a composite category enabling all Minerium-related debug output
                if(ptrCategory->count(std::string("minerium"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Minerium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Minerium
    // Mac: ~/Library/Application Support/Minerium
    // Unix: ~/.mineriumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Minerium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Minerium";
#else
    // Unix
    return pathRet / ".mineriumcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override minerium.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", MINERIUM_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Minerium Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Minerium Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2009, nEndYear) + "The Minerium Core developers";
    }
