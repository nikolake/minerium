...
const int MINERIUM_TX_VERSION = 0x7400;
...
bool DecodeAndParseMineriumTx(const CTransaction& tx, int& op, std::vector<std::vector<unsigned char> >& vvch, char &type);
...
std::string GenerateMineriumGuid();
...
bool GetMineriumData(const CTransaction &tx, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash, int& nOut);
bool GetMineriumData(const CScript &scriptPubKey, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash);
bool IsSysServiceExpired(const uint64_t &nTime);
bool GetTimeToPrune(const CScript& scriptPubKey, uint64_t &nTime);
bool IsMineriumScript(const CScript& scriptPubKey, int &op, std::vector<std::vector<unsigned char> > &vvchArgs);
...
void CleanupMineriumServiceDatabases(int &servicesCleaned);
void GetAddress(const CAliasIndex &alias, CMineriumAddress* address, CScript& script, const uint32_t nPaymentOption=1);
std::string GetMineriumTransactionDescription(const CTransaction& tx, const int op, std::string& responseEnglish, const char &type, std::string& responseGUID);
...
UniValue mineriumtxfund_helper(const std::vector<unsigned char> &vchAlias, const std::vector<unsigned char> &vchWitness, const CRecipient &aliasRecipient, std::vector<CRecipient> &vecSend);
