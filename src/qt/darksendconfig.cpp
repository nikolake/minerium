...
void DarksendConfig::clickBasic()
{
    ...
    QString strAmount(MineriumUnits::formatWithUnit(
        model->getOptionsModel()->getDisplayUnit(), 100000 * COIN));
    QMessageBox::information(this, tr("PrivateSend Configuration"),
        tr(
            "PrivateSend was successfully set to basic (%1 and 2 rounds). You can change this at any time by opening Minerium's configuration screen."
        ).arg(strAmount)
    );
...
void DarksendConfig::clickHigh()
{
    ...
    QMessageBox::information(this, tr("PrivateSend Configuration"),
        tr(
            "PrivateSend was successfully set to high (%1 and 8 rounds). You can change this at any time by opening Minerium's configuration screen."
        ).arg(strAmount)
    );
...
void DarksendConfig::clickMax()
{
    ...
    QMessageBox::information(this, tr("PrivateSend Configuration"),
        tr(
            "PrivateSend was successfully set to maximum (%1 and 16 rounds). You can change this at any time by opening Minerium's configuration screen."
        ).arg(strAmount)
    );
