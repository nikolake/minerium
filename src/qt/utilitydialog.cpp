    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (helpMode == about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  minerium-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_MINERIUM_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", MineriumGUI::DEFAULT_UIPLATFORM));
        }
...
All the Minerium in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.1 MINR, 1 MINR, 10 MINR and 100 MINR -- sort of like the paper money you use every day.</li> \
...
For more info see <a href=\"https://mineriumpay.atlassian.net/wiki/display/DOC/PrivateSend\">https://mineriumpay.atlassian.net/wiki/display/DOC/PrivateSend</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
...
QWidget *ShutdownWindow::showShutdownWindow(MineriumGUI *window)
{
    if (!window)
        return nullptr;
