...
const std::string MineriumGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MineriumGUI::DEFAULT_WALLET = "~Default";

MineriumGUI::MineriumGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void MineriumGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Minerium address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and minerium: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about Minerium Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Minerium addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Minerium addresses"));
    ...
    openAction->setStatusTip(tr("Open a minerium: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Minerium command-line options").arg(tr(PACKAGE_NAME)));
...
void MineriumGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void MineriumGUI::createToolBars()
{
#ifdef ENABLE_WALLET
    if(walletFrame)
    {
...
void MineriumGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool MineriumGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MineriumGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MineriumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MineriumGUI::setWalletActionsEnabled(bool enabled)
{
...
void MineriumGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
...
void MineriumGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void MineriumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MineriumGUI::optionsClicked()
{
...
void MineriumGUI::showDebugWindow()
{
...
void MineriumGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void MineriumGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MineriumGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void MineriumGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void MineriumGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void MineriumGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void MineriumGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void MineriumGUI::showBackups()
{
    GUIUtil::showBackups();
}

void MineriumGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void MineriumGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void MineriumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MineriumGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MineriumGUI::gotoMasternodePage()
{
    QSettings settings;
    if (!fLiteMode && settings.value("fShowMasternodesTab").toBool() && masternodeAction) {
...
void MineriumGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MineriumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MineriumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MineriumGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void MineriumGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Minerium network", "", count));
    } else {
...
void MineriumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MineriumGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MineriumGUI::updateHeadersSyncProgressLabel()
{
...
void MineriumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in minerium
        {
...
void MineriumGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void MineriumGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Minerium Core"); // default title
...
void MineriumGUI::changeEvent(QEvent *e)
{
...
void MineriumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MineriumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MineriumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(MineriumUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void MineriumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MineriumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MineriumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MineriumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MineriumGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void MineriumGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void MineriumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MineriumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MineriumGUI::detectShutdown()
{
...
void MineriumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MineriumGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MineriumGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MineriumGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void MineriumGUI::subscribeToCoreSignals()
{
...
void MineriumGUI::unsubscribeFromCoreSignals()
{
...
void MineriumGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void MineriumGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<MineriumUnits::Unit> units = MineriumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const MineriumUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(MineriumUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(MineriumUnits::Unit u, MineriumUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(MineriumUnits::name(u)), this);
...
{
    setText(MineriumUnits::name(newUnits));
}
