...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total minerium balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized minerium balance of the wallet\n"
	...
	obj.push_back(Pair("version", MINERIUM_CLIENT_VERSION));
	// MINERIUM
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "minerium|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use '+' to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "minerium")
            + HelpExampleRpc("debug", "minerium+net")
        );
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CMineriumAddress(addr).ToString());
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given minerium address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The minerium address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
			"  \"address\" : \"mineriumaddress\", (string) The minerium address validated\n"
			// MINERIUM
			"  \"zaddress\" : \"mineriumaddress\", (string) The zcash t-addr associated with this minerium address validated\n"
			"  \"saddress\" : \"mineriumaddress\", (string) The Minerium 3 S address scheme associated with this minerium address validated\n"
			"  \"btcaddress\" : \"mineriumaddress\", (string) The bitcoin address associated with this minerium address validated\n"
			"  \"alias\" : \"mineriumalias\", (string) The minerium alias associated with this minerium address validated\n"
...
#ifdef ENABLE_WALLET
		// Case 1: Minerium address and we have full public key:
		CMineriumAddress address(ks);
		// MINERIUM
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are minerium addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) minerium address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
	...
	addr.Set(dest, CChainParams::ADDRESS_MINR);
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The minerium address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CMineriumAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CMineriumAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CMineriumAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CMineriumAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "minerium-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Minerium features */
    { "minerium",               "mnsync",                 &mnsync,                 true,  {} },
    { "minerium",               "spork",                  &spork,                  true,  {"value"} },
