...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("outpoint",      mnInfo.outpoint.ToStringShort()));
        obj.push_back(Pair("payee",         CMineriumAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "genkey")
    {
        ...
        return CMineriumSecret(secret).ToString();
    }
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", CMineriumAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Minerium address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (const auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CMineriumAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                objMN.push_back(Pair("payee", CMineriumAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
                ...
                CMineriumAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode")
    {
        ...
        for (const auto& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CMineriumAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CMineriumAddress(mnb.pubKeyMasternode.GetID()).ToString()));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "minerium",               "masternode",             &masternode,             true,  {} },
    { "minerium",               "masternodelist",         &masternodelist,         true,  {} },
    { "minerium",               "masternodebroadcast",    &masternodebroadcast,    true,  {} },
    { "minerium",               "getpoolinfo",            &getpoolinfo,            true,  {} },
    { "minerium",               "sentinelping",           &sentinelping,           true,  {} },
#ifdef ENABLE_WALLET
    { "minerium",               "privatesend",            &privatesend,            false, {} },
#endif // ENABLE_WALLET
};
