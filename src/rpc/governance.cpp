...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in minerium.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("minerium.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Minerium features */
    { "minerium",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "minerium",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "minerium",               "gobject",                &gobject,                true,  {} },
    { "minerium",               "voteraw",                &voteraw,                true,  {} },
