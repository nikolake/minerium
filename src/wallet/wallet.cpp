...
// file COPYING or http://www.opensource.org/licenses/mit-license.php. like 574 + 4873
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CMineriumAddress(CScriptID(redeemScript)).ToString();
...
void CWallet::AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseInstantSend, bool fIncludeMineriumAliasBalances, bool fIncludeMineriumAliasOutputs) const
{
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
				// MINERIUM
				if (!fIncludeMineriumAliasBalances || !fIncludeMineriumAliasOutputs) {
					if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs && !coinControl->IsSelected(COutPoint((*it).first, i)))
						continue;
					// MINERIUM txs are unspendable by wallet unless using coincontrol(and the tx is selected)
					if (!coinControl || !coinControl->IsSelected(COutPoint((*it).first, i)))
					{
						...
						if (pcoin->tx->vout.size() >= i && ExtractDestination(pcoin->tx->vout[i].scriptPubKey, sysdestination))
						{
							...
							if (IsMineriumScript(pcoin->tx->vout[i].scriptPubKey, op, vvchArgs) && !fIncludeMineriumAliasOutputs)
								continue;
							CMineriumAddress address = CMineriumAddress(sysdestination);
							if (DoesAliasExist(address.ToString()) && !fIncludeMineriumAliasBalances)
								continue;
						}
					}
				}
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxDSIn>& vecTxDSInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax)
{
	...
	// MINERIUM include minerium alias balances
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed) const
{
    ...
    if (LogAcceptCategory("selectcoins")) {
        ...
        for (const auto& item : vecTallyRet)
            strMessage += strprintf("  %s %f\n", CMineriumAddress(item.txdest).ToString().c_str(), float(item.nAmount)/COIN);
...
bool CWallet::GetMasternodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, const std::string& strTxHash, const std::string& strOutputIndex)
{
	...
	// MINERIUM include sys alias balances
...
bool CWallet::GetOutpointAndKeysFromOutput(const COutput& out, COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet)
{
    ...
    CMineriumAddress address2(address1);
...
{
    std::vector<COutput> vCoins;
	// MINERIUM include alias balances
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 MINR."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-minerium-address
                        CScript scriptChange;
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CMineriumAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CMineriumAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CMineriumAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CMineriumAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CMineriumAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CMineriumAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CMineriumAddress(dest).ToString(), key);
}
