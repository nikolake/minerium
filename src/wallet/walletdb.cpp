             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            ...
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CMineriumAddress(strAddress).Get()].name;
        }
        else if (strType == "purpose")
        {
            std::string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CMineriumAddress(strAddress).Get()].purpose;
        }
        else if (strType == "tx")
        {
            uint256 hash;
			...
			{
				// MINERIUM
				if (wtx.tx->GetHash() != hash && wtx.tx->nVersion == MINERIUM_TX_VERSION)
					return true;
        ...
        else if (strType == "destdata")
        {
            ...
            ssKey >> strKey;
            ssValue >> strValue;
            if (!pwallet->LoadDestData(CMineriumAddress(strAddress).Get(), strKey, strValue))
            {
