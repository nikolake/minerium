 ...
 * between mineriumd, and minerium-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), MINERIUM_CONF_FILENAME));
    if (mode == HMM_MINERIUMD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), MINERIUM_PID_FILENAME));
#endif
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "minerium (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_MINERIUM_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Minerium specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N MINR anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
	...
	return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
		FormatParagraph(strprintf(_("Copyright (C) 2020-%i The Minerium Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
		...
		"\n" +
		FormatParagraph(_("Minerium is open source software produced by a global network of developers. By downloading, distributing and using Minerium and the Minerium network you release the developers involved in the Minerium Project past, present, and future from any and all liability. You are responsible for your creations on the Minerium network. You agree that the developers of the Minerium Project carry no responsibility for the actions/data or entities of *any* definition created on the network by yourself or others on the network to which you may be exposed.")) + "\n" +
		"\n" +
		FormatParagraph(_("The developers of the Minerium Project do not have the power to modify data on the Minerium network, it is backed by an immutable blockchain, which you further acknowledge through use of Minerium, the Minerium network, and Minerium services. If you do not agree to these terms, please refrain from using Minerium and its related services.")) +
		"\n";
}
...
/** Sanity checks
 *  Ensure that Minerium Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Minerium Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Minerium Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", MINERIUM_CONF_FILENAME)).string());
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
				...
				// MINERIUM Check for changed -addressindex state
    ...
    //lite mode disables all Minerium-specific functionality
    ...
    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/mineriumpay/minerium/pull/1817 and https://github.com/mineriumpay/minerium/pull/1743
    ...
    if(fMasternodeMode) {
        ...
        if(!strMasterNodePrivKey.empty()) {
            ...
            LogPrintf("  pubKeyMasternode: %s\n", CMineriumAddress(activeMasternode.pubKeyMasternode.GetID()).ToString());
        } else {
    ...
    // ********************************************************* Step 11c: update block tip in Minerium modules
    ...
    // ********************************************************* Step 11d: schedule Minerium-specific tasks
