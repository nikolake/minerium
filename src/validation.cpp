...
// Copyright (c) 2020-2020 The Minerium Core developers
...
#if defined(NDEBUG)
# error "Minerium Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Minerium Signed Message:\n";
...
// MINERIUM
bool CheckMineriumInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, const CBlock& block, bool bSanity)
{
	...
	if (block.vtx.empty() && tx.nVersion == MINERIUM_TX_VERSION) {
		...
		if (good)
		{
			...
			return state.DoS(100, false, REJECT_INVALID, "minerium-inputs-error");
		...
		if (CreateGraphFromVTX(nHeight, sortedBlock.vtx, graph, vertices, mapTxIndex)) {
			...
			if (!sortedBlock.vtx.empty()) {
				if (!DAGTopologicalSort(sortedBlock.vtx, conflictedIndexes, graph, mapTxIndex)) {
					if (fDebug)
						LogPrintf("CheckMineriumInputs: Toposort failed");
					return true;
				}
			}
		}
		...
		nFlushIndexBlocks++;
		if ((nFlushIndexBlocks % 200) == 0 && !FlushMineriumDBs())
			return state.DoS(0, false, REJECT_INVALID, "Failed to flush minerium databases");
	}
	return true;
}
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun, bool bMultiThreaded)
{
		...
		BOOST_FOREACH(const CTxIn &txin, tx.vin)
		{
			...
			if (itConflicting != pool.mapNextTx.end())
			{
				...
				if (!setConflicts.count(ptxConflicting->GetHash()))
				{
					...
					// MINERIUM txs are not replacable
					else if (ptxConflicting->nVersion == MINERIUM_TX_VERSION) {
						return state.DoS(0, error("AcceptToMemoryPool : Minerium Transaction %s conflicts with Transaction request %s",
							hash.ToString(), ptxConflicting->GetHash().ToString()),
...

	{
		...
		if (!bMultiThreaded) {
			...
			if (!control.Wait())
				return false;
			if (!CheckMineriumInputs(tx, state, view, true, chainActive.Height(), CBlock())) {
				return false;
			}
		}
		...
		if (bMultiThreaded && threadpool != NULL)
		{
			...
			static int totalMineriumCheckCount = 0;
			static int totalMineriumCheckMicros = 0;
			...
			std::packaged_task<void()> task([&pool, ptx, hash, coins_to_uncache, hashCacheEntry, vChecks]() {
				...
				int thisMineriumCheckCount = 0;
				int thisCheckMicros = 0;
				int thisMineriumCheckMicros = 0;
				...
				if (isCheckPassing)
				{
					int64_t mineriumCheckTime;
					if (fLogThreadpool) {
						mineriumCheckTime = GetTimeMicros();
						thisMineriumCheckCount += 1;
					}
					if (!CheckMineriumInputs(txIn, validationState, coinsViewCache, true, chainActive.Height(), CBlock()))
					{
						nLastMultithreadMempoolFailure = GetTime();
						LOCK2(cs_main, mempool.cs);
						LogPrint("mempool", "%s: %s\n", "CheckMineriumInputs Error", hash.ToString());
						BOOST_FOREACH(const COutPoint& hashTx, coins_to_uncache)
							pcoinsTip->Uncache(hashTx);
						pool.removeRecursive(txIn, MemPoolRemovalReason::UNKNOWN);
						pool.ClearPrioritisation(hash);
					...
					if (fLogThreadpool) {
						// how long did we run CheckMineriumInputs()'s
						thisMineriumCheckMicros = GetTimeMicros() - mineriumCheckTime;
					}
				}
				if (fLogThreadpool) {
					// metrics
					...
					totalMineriumCheckCount += thisMineriumCheckCount;
					totalMineriumCheckMicros += thisMineriumCheckMicros;
			...
			if (fLogThreadpool && totalCheckCount > 0 && totalMineriumCheckCount > 0 && totalExecutionCount > 0 && (!fUnitTest || (fUnitTest && totalExecutionCount % 100 == 0))) {
				int avgCheckMicros = totalCheckMicros / totalCheckCount;
				int avgMineriumCheckMicros = totalMineriumCheckMicros / totalMineriumCheckCount;
				...
				const std::string &messageInternals = "THREADPOOL::%s:Signature check internals - check(%d): total %lld, avg %lld microseconds, minerium(%d): total %lld, avg %lld microseconds\n";
				...
				LogPrint("threadpool", messageInternals, hash.ToString(), totalCheckCount, totalCheckMicros, avgCheckMicros, totalMineriumCheckCount, totalMineriumCheckMicros, avgMineriumCheckMicros);
...
{
	// MINERIUM if its been less 60 seconds since the last MT mempool verification failure then fallback to single threaded
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, CAmount &nTotalRewardWithMasternodes, bool fSuperblockPartOnly, bool fMasternodePartOnly, unsigned int nStartHeight)
{
    ...
    else if (nHeight >= 51 && nHeight <= 10080)
    {
        nSubsidy = 6 * COIN;
    }
    else if (nHeight >= 10081 && nHeight <= 10320)
    {
        nSubsidy = 126 * COIN;
    }
    else if (nHeight >= 10321 && nHeight <= 10560)
    {
        nSubsidy = 226 * COIN;
    }
    else if (nHeight >= 10561 && nHeight <= 10800)
    {
        nSubsidy = 1006 * COIN;
    }
    else if (nHeight >= 10801 && nHeight <= 11040)
    {
        nSubsidy = 2746 * COIN;
    }
    else if (nHeight >= 11041 && nHeight <= 11280)
    {
        nSubsidy = 416 * COIN;
    }
    else if (nHeight >= 11281 && nHeight <= 11520)
    {
        nSubsidy = 506 * COIN;
    }
    else if (nHeight >= 60481 && nHeight <= 61920)
    {
        nSubsidy = 256 * COIN;
    }
    else if (nHeight >= 97921 && nHeight <= 99360)
    {
        nSubsidy = 46 * COIN;
    }
    else if (nHeight >= 156961 && nHeight <= 158400)
    {
        nSubsidy = 116 * COIN;
    }
    else if (nHeight >= 208801 && nHeight <= 210240)
    {
        nSubsidy = 86 * COIN;
    }
    else if (nHeight >= 234721 && nHeight <= 236160)
    {
        nSubsidy = 666 * COIN;
    }
    else if (nHeight >= 270721 && nHeight <= 272160)
    {
        nSubsidy = 316 * COIN;
    }
    else if (nHeight >= 308161 && nHeight <= 309600)
    {
        nSubsidy = 526 * COIN;
    }
    else if (nHeight >= 365761 && nHeight <= 367200)
    {
        nSubsidy = 116 * COIN;
    }
    else if (nHeight >= 411841 && nHeight <= 413280)
    {
        nSubsidy = 336 * COIN;
    }
    else if (nHeight >= 470881 && nHeight <= 472320)
    {
        nSubsidy = 766 * COIN;
    }
    else if (nHeight >= 491041 && nHeight <= 492480)
    {
        nSubsidy = 676 * COIN;
    }
    else if (nHeight >= 532801 && nHeight <= 534240)
    {
        nSubsidy = 316 * COIN;
    }
    else if (nHeight >= 564481 && nHeight <= 565920)
    {
        nSubsidy = 286 * COIN;
    }
    else if (nHeight >= 627481 && nHeight <= 629280)
    {
        nSubsidy = 446 * COIN;
    }
    else if (nHeight >= 673921 && nHeight <= 675360)
    {
        nSubsidy = 666 * COIN;
    }
    else if (nHeight >= 718561 && nHeight <= 720000)
    {
        nSubsidy = 306 * COIN;
    }
    else if (nHeight >= 743041 && nHeight <= 744480)
    {
        nSubsidy = 176 * COIN;
    }
    else if (nHeight >= 793441 && nHeight <= 794880)
    {
        nSubsidy = 326 * COIN;
    }
    else if (nHeight >= 839521 && nHeight <= 840960)
    {
        nSubsidy = 196 * COIN;
    }
    else if (nHeight >= 889921 && nHeight <= 891360)
    {
        nSubsidy = 449 * COIN;
    }
    else if (nHeight >= 920161 && nHeight <= 921600)
    {
        nSubsidy = 686 * COIN;
    }
    else if (nHeight >= 973441 && nHeight <= 974880)
    {
        nSubsidy = 166 * COIN;
    }
    else if (nHeight >= 1002241 && nHeight <= 1003680)
    {
        nSubsidy = 56 * COIN;
    }
    else if (nHeight >= 1049761 && nHeight <= 1051200)
    {
        nSubsidy = 2886 * COIN;
    }
    else
    ...
    {
        if (nHeight >= 2 && nHeight <= 10080)
            nSubsidy *= 1.00;
        else if (nHeight >= 10081 && nHeight <= 10800)
            nSubsidy *= 0.20;
        else if (nHeight >= 10801 && nHeight <= 61920)
            nSubsidy *= 0.35;
        else if (nHeight >= 61921 && nHeight <= 234720)
            nSubsidy *= 0.25;
        else if (nHeight >= 234721 && nHeight <= 236160)
            nSubsidy *= 0.50;
        else if (nHeight >= 236161 && nHeight <= 308160)
            nSubsidy *= 0.25;
        else if (nHeight >= 308161 && nHeight <= 367200)
            nSubsidy *= 0.20;
        else if (nHeight >= 367201 && nHeight <= 534240)
            nSubsidy *= 0.35;
        else if (nHeight >= 534241 && nHeight <= 1049760)
            nSubsidy *= 0.25;
        else if (nHeight >= 1049761 && nHeight <= 1051200)
            nSubsidy *= 0.50;
        else
        ...
        if (fAddressIndex) {
            ...
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
					...
					if (RemoveMineriumScript(out.scriptPubKey, scriptPubKeyOut))
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (fAddressIndex) {
                    ...
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
						...
						CScript scriptOut;
						CScript scriptPubKeyOut;
						if (RemoveMineriumScript(prevout.scriptPubKey, scriptPubKeyOut))
							scriptOut = scriptPubKeyOut;
						else
...
void ThreadScriptCheck() {
    RenameThread("minerium-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
	...
	if (!CheckMineriumInputs(*block.vtx[0], state, viewOld, fJustCheck, pindex->nHeight, block))
		return error("ConnectBlock(): CheckMineriumInputs on block %s failed\n",
			block.GetHash().ToString());
    int64_t nTime3 = GetTimeMicros(); nTimeConnect += nTime3 - nTime2;
    LogPrint("bench", "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs]\n", (unsigned)block.vtx.size(), 0.001 * (nTime3 - nTime2), 0.001 * (nTime3 - nTime2) / block.vtx.size(), nInputs <= 1 ? 0 : 0.001 * (nTime3 - nTime2) / (nInputs-1), nTimeConnect * 0.000001);

    // MINERIUM : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
	...
	if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, nFees, nTotalRewardWithMasternodes)) {
		...
		return state.DoS(0, error("ConnectBlock(MINR): couldn't find masternode or superblock payments"),
			REJECT_INVALID, "bad-cb-payee");
	}

    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, nTotalRewardWithMasternodes, nFees, strError)) {
        return state.DoS(0, error("ConnectBlock(MINR): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    // END MINERIUM
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    if (fDoFullFlush) {
		...
		// MINERIUM
		if (!FlushMineriumDBs())
			return AbortNode(state, "Failed to flush minerium databases");
        nLastFlush = nNow;
    }
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
			// MINERIUM, skip auxpow bit 8 or dummy bit 28
			if (bit == 8 || bit == 28)
				continue;
...
{
	// MINERIUM check auxpow first then fallback to normal check
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // MINR : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(MINR): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // MINERIUM check for version 2, 3 and 4 upgrades
