...
###   mineriumcore.org and minerium.org and compares them.
...
WORKINGDIR="/tmp/minerium_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://mineriumcore.org"
HOST2="https://minerium.org"
BASEDIR="/bin/"
VERSIONPREFIX="minerium-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'minerium-',
...
if [ $? -ne 0 ]; then
   echo "minerium.org failed to provide signature file, but mineriumcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "minerium.org and mineriumcore.org signature files were not equal?"
