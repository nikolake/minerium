UNIX BUILD NOTES
====================
Some notes on how to build Minerium Core in Unix.
...
Note
---------------------
Always use absolute paths to configure and compile minerium and the dependencies,
...
This will build minerium-qt as well if the dependencies are met.
...
memory available when compiling Minerium Core. On systems with less, gcc can be
...
`--with-incompatible-bdb` to configure. See the section "Disable-wallet mode" to build Minerium 
Core without wallet.
...
>**Ubuntu:** db4.8 packages are available [here](https://launchpad.net/~minerium/+archive/minerium).
...
If you want to build Minerium-Qt, make sure that the required packages for Qt development are 
...
Once these are installed, they will be found by configure and a minerium-qt executable will be
built by default.
...
The version of Boost that ships with CentOS is too old to work with Minerium - version 1.55+ 
...
to your `.bashrc` as well to configure this variable on when starting `mineriumd`.
...
Notes
-----
The release is built with GCC and then "strip mineriumd" to strip the debug symbols, 
...
so from the root of the Minerium repository.

```shell
./contrib/install_db4.sh `pwd`
```

When compiling Minerium you will need to run `./configure` with the results of this script.
...
Security
--------
To help make your minerium installation more secure by making certain attacks impossible to
...
* Non-executable Stack
  ...
  vulnerable buffers are found. By default, minerium should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, minerium may be compiled in 
...
[PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/minerium/trunk/PKGBUILD). 
As mentioned above, when maintaining portability of the wallet between the standard Minerium 
